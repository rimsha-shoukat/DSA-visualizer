...
export default function Signup({ setLog, setSign }) {
    const [form, setForm] = useState({ name: "", email: "", password: "" });  // Fixed: 'password' not 'pass'
    const [message, setMessage] = useState('');
    const [isError, setIsError] = useState(false);  // To distinguish error/success
    ...
    const handleFormSubmission = async (e) => {
        ...
        try {
            const res = await fetch('/signup-action', {  // Fixed: Full path, not './'
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(form),  // Now sends {name, email, password}
            });

            const data = await res.json();

            if (!res.ok) {  // Fixed: Handle 400/409/500 errors properly
    ...
    return (
        <>
            ...
            <form onSubmit={(e) => handleFormSubmission(e)} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[24rem] max-[470px]:w-[18rem] h-auto flex flex-col gap-4 items-center justify-center p-6 backdrop-blur-md bg-white/30 rounded-md">
                ...
                <input
                    className="rounded-full w-[100%] px-4 py-2 border-none ring-2 ring-gray-300 placeholder-gray-700 focus:outline-none focus:border-none focus:ring-2 focus:ring-blue-800"
                    value={form.password}  // Fixed: 'password' not 'pass'
