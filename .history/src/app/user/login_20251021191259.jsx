...
export default function login({ setLog, setSign, setBoard }) {
    ...
    const handleFormSubmission = async (e) => {
        ...
        if(users.find(user => user.email.toString() !== form.email)){
            setMessage('User not found. Please sign up first.');
            setIsError(true);
            return;
        }

        if(users.find(user => user.email.toString() === form.email && user.password.toString() !== form.password)){
            setMessage("Invalid email or password");
            setIsError(true);
            return;
        }

        if(users.find(user => user.email.toString() === form.email && user.password === form.password)){
