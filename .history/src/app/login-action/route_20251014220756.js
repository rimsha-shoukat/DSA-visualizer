"use server";

import { connectDB, User } from '@/database';  // Adjust path if needed
import { NextResponse } from 'next/server';
import bcrypt from 'bcrypt';
import Joi from 'joi';  // For input validation (already installed)

const loginSchema = Joi.object({
  email: Joi.string().email({ tlds: { allow: false } }).required().messages({
    'string.email': 'Invalid email address',
    'any.required': 'Email is required',
  }),
  password: Joi.string().min(8).required().messages({
    'string.min': 'Password must be at least 8 characters long',
    'any.required': 'Password is required',
  }),
});

export async function POST(request) {
  try {
    const body = await request.json();
    const { email, password } = body;

    // Early check for missing fields
    if (!email || !password) {
      return NextResponse.json({ message: 'Missing required fields (email, password)' }, { status: 400 });
    }

    // Validate with Joi
    const { error, value } = loginSchema.validate(body, { abortEarly: false });
    if (error) {
      const messages = error.details.map((detail) => detail.message).join(', ');
      return NextResponse.json({ message: messages }, { status: 400 });
    }

    const { email: validatedEmail, password: validatedPassword } = value;

    // Connect to DB
    await connectDB();

    // Find user by email
    const user = await User.findOne({ email: validatedEmail });
    if (!user) {
      return NextResponse.json({ message: 'Invalid email or password' }, { status: 401 });  // Don't specify which is wrong for security
    }

    // Compare passwords
    const isMatch = await bcrypt.compare(validatedPassword, user.password);
    if (!isMatch) {
      return NextResponse.json({ message: 'Invalid email or password' }, { status: 401 });
    }

    // Success: In a real app, generate JWT/session here
    return NextResponse.json({ message: 'Login successful!', user: { id: user._id, name: user.name, email: user.email } }, { status: 200 });
  } catch (err) {
    console.error('Login error details:', err.message || err);
    return NextResponse.json({ message: 'Error while logging in. Please try again.' }, { status: 500 });
  }
}