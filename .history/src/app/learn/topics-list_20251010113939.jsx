...
export default function topicsList({setList}){
    ...
    useEffect(()=>{
        ...
        const searchTopics = Topics.filter((t) =>
            t.title.toLowerCase().includes(searchRef.current.value.toLowerCase()));
        if (searchTopics.length > 0) {
            setFilterTopics(searchProducts);
        }
        }
    },[topic]);
    ...
    return (
        <>
            ...
            <div className="w-[24rem] h-auto absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 backdrop-blur-md rounded-md bg-white/30 transition-all duration-300 flex flex-col items-center justify-center gap-8 p-6">
                <input ref={searchRef} onChange={(e)=> setTopic(e.target.value)} value={topic} className="rounded-full w-[100%] px-4 py-2 border-none ring-2 ring-gray-300 placeholder-gray-700 focus:outline-none focus:border-none focus:ring-2 focus:ring-blue-800" placeholder="Search" type="text" />
