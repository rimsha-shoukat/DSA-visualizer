...
export default function topicsList({setList}){
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-2 w-[100%] h-[22rem] overflow-y-scroll [scrollbar-width:none] [&::-webkit-scrollbar]:hidden transition-all duration-300">
                {
                    error && <p className="text-red-700 text-sm">{error}</p>
                }
                {
                    filterTopics.map((t)=> (
                        <Link href={`/learn/topic/${t.id}`} key={t.id} onClick={()=> setList(false)}>
